...
@interface PanoLog(privatedMethod)
...
+ (NSString*)KKLogFormatPrefix:(KKLogLevel)logLevel
{
    return [NSString stringWithFormat:@"[%@] ", [PanoLog KKStringFromLogLevel:logLevel]];
}
...
+ (void)logIntial
{
    if (!logFilePath)
    {
        ...
        NSString *logDirectory       = [documentsDirectory stringByAppendingString:@"/applog/"];
        NSString *crashDirectory     = [documentsDirectory stringByAppendingString:@"/applog/"];
...
+ (void)logvLevel:(KKLogLevel)level Format:(NSString *)format VaList:(va_list)args
{
    ...
    dispatch_async(k_operationQueue, ^{
        ...
        if (level >= LogLevel)
        {
            formatTmp = [[PanoLog KKLogFormatPrefix:level] stringByAppendingString:formatTmp];
            ...
            NSString *content = [NSString stringWithFormat:@"%@ %@", [self.dateFormatter stringFromDate:[NSDate date]], contentN];
...
+ (void)logCrash:(NSException *)exception
{
    ...
    NSString *fileName = [NSString stringWithFormat:@"CRASH_%@.log", [[PanoLog nowBeijingTime] description]];
    ...
    content = [content stringByAppendingString:[NSString stringWithFormat:@"iPhone:%@  iOS Version:%@ Language:%@",[PanoLog platformString], [[UIDevice currentDevice] systemVersion],phoneLanguage]];
    NSError *error = nil;
...
#endif
        [PanoLog logE:@"CRASH LOG CREAT ERR INFO IS %@",error];
    }
...
+ (void)logLevel:(KKLogLevel)level LogInfo:(NSString *)format, ...
{
    ...
    [PanoLog logvLevel:level Format:format VaList:args];
    va_end(args);
}
...
+ (void)logV:(NSString *)format, ...
{
    va_list args;
    va_start(args, format);
    [PanoLog logvLevel:LOGLEVELV Format:format VaList:args];
    va_end(args);
}
...
+ (void)logD:(NSString *)format, ...
{
    va_list args;
    va_start(args, format);
    [PanoLog logvLevel:LOGLEVELD Format:format VaList:args];
    va_end(args);
}
...
+ (void)logI:(NSString *)format, ...
{
    va_list args;
    va_start(args, format);
    [PanoLog logvLevel:LOGLEVELI Format:format VaList:args];
    va_end(args);
}
...
+ (void)logW:(NSString *)format, ...
{
    va_list args;
    va_start(args, format);
    [PanoLog logvLevel:LOGLEVELW Format:format VaList:args];
    va_end(args);
}
...
+ (void)logE:(NSString *)format, ...
{
    va_list args;
    va_start(args, format);
    [PanoLog logvLevel:LOGLEVELE Format:format VaList:args];
    va_end(args);
}
...
{
    NSString *platform = [PanoLog platform];
